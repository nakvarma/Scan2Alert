  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <main className="flex-1 px-4 py-6 w-full max-w-7xl mx-auto">
        <h1 className="text-2xl md:text-3xl font-bold text-center mb-6 text-black">Car Vehicle Details</h1>
        ...
        {userDetails && (
          <>
            <h2 className="text-xl md:text-2xl font-semibold text-center text-black uppercase">
              {userDetails.name}
            </h2>
            <p className="text-center text-base md:text-lg mt-1">{userDetails.phone}</p>

            <h3 className="mt-6 text-lg md:text-xl font-semibold">Vehicles</h3>

            {filterVehicles().map((vehicle: any, index: number) => (
              <div key={index} className="mt-6 border-b pb-4 shadow-md rounded-md bg-white p-4">
                <div
                  className="flex items-center justify-between cursor-pointer"
                  onClick={() => toggleVehicleDetails(index)}
                >
                  <h4 className="text-base md:text-lg font-medium text-black">{vehicle.number}</h4>
                ...
                {selectedVehicle === index && (
                  <div className="mt-4 space-y-3">
                    ...
                    {vehicle.dispatchAddress && (
                      <p><strong>DispatchAddress:</strong> {vehicle.dispatchAddress}</p>
                    )}
