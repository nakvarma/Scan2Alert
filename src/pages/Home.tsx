            ...
            <div className="flex-1 overflow-y-auto px-4 sm:px-6 md:px-8 py-6">
                <div className="max-w-6xl mx-auto bg-white shadow-xl rounded-2xl p-8 md:p-10 space-y-12 transform hover:scale-105 transition-all duration-700 ease-in-out">
                    <div className="text-center">
                        <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-teal-600 mb-6">Welcome to the Car Registration Portal</h1>
                        <p className="text-teal-500 text-lg sm:text-xl mb-6">Easily register your vehicle online in just a few steps.</p>
                        <motion.button
                            className="bg-teal-500 text-white px-10 py-4 text-lg rounded-2xl shadow-md transition-all duration-500 hover:bg-teal-600 hover:scale-110 hover:shadow-xl"
                            whileHover={{ scale: 1.1, rotate: 5, backgroundColor: '#4FD1C5', boxShadow: '0 15px 40px rgba(79, 209, 197, 0.6)' }}
                            whileTap={{ scale: 0.95, rotate: -5, backgroundColor: '#38B2AC', boxShadow: '0 5px 20px rgba(79, 209, 197, 0.4)' }}
                        >
                            Start Registration
                        </motion.button>
                    </div>
                    ...
                    <div className="relative w-full overflow-hidden bg-teal-50 p-8 rounded-2xl shadow-xl">
                        <h2 className="text-2xl font-semibold text-teal-600 text-center mb-6">What Makes Us Different?</h2>
                        <div className="flex flex-wrap gap-8 justify-center">
                            {features.map((item, index) => (
                                <motion.div
                                    ...
                                    whileHover={{ scale: 1.1, boxShadow: '0 20px 40px rgba(79, 209, 197, 0.5)' }}
                                    whileTap={{ scale: 0.95, boxShadow: '0 15px 30px rgba(79, 209, 197, 0.4)' }}
                                >
                                    <h3 className="text-xl font-semibold text-teal-600">{item.title}</h3>
                                    <p className="mt-2 text-teal-700">{item.text}</p>
                                </motion.div>
                            ))}
                        </div>
                    </div>
